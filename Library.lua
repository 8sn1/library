...
do
	...
	function Funcs:AddColorPicker(Idx, Info)
		...
		local DisplayFrame = Library:Create('Frame', { BackgroundColor3 = ColorPicker.Value; BorderColor3 = Library:GetDarkerColor(ColorPicker.Value); BorderMode = Enum.BorderMode.Inset; Size = UDim2.new(0, 28, 0, 14); ZIndex = 6; Parent = ToggleLabel; });
		...
		DisplayFrame.InputBegan:Connect(function(Input) if Input.UserInputType == Enum.UserInputType.MouseButton1 and not Library:MouseIsOverOpenedFrame() then if PickerFrameOuter.Visible then ColorPicker:Hide() else ColorPicker:Show() end; elseif Input.UserInputType == Enum.UserInputType.MouseButton2 and not Library:MouseIsOverOpenedFrame() then ColorPicker:Hide() end end);
		...
		if (self.ToggleRegion) then self.ColorPickerCount += 1; if (self.ColorPickerCount > 2) then self.ToggleRegion.Size -= UDim2.new(0,32,0,0); end end
		return self;
	end;
	...
	function Funcs:AddKeyPicker(Idx, Info)
		...
		function KeyPicker:Update()
			if not KeyPicker.NoUI then
				...
				if (mode == "Active" and KeyPicker.Parent.Type == "Toggle" and (not State or not KeyPicker.Parent.Value)) then ContainerLabel.Visible = false;
				elseif (mode == "Toggled" and KeyPicker.Parent.Type == "Toggle" and not KeyPicker.Parent.Value) then ContainerLabel.Visible = false;
				else update(State); end;
			else ContainerLabel.Visible = false; end;
...
do
	...
	function Funcs:AddToggle(Idx, Info)
		...
		local Blanks = { };
		local Toggle = { Value = Info.Default or false; Type = 'Toggle'; Callback = Info.Callback or function(Value) end; Addons = {}, Risky = Info.Risky, Idx = Idx; };
		local Groupbox = self; local Container = Groupbox.Container;
		local ToggleOuter = Library:Create('Frame', { BackgroundColor3 = Color3.new(0, 0, 0); BorderColor3 = Color3.new(0, 0, 0); Size = UDim2.new(0, 13, 0, 13); ZIndex = 5; Parent = Container; });
		...
		local ToggleLabel = Library:CreateLabel({ Size = UDim2.new(0, 216, 1, 0); Position = UDim2.new(1, 6, 0, 0); TextSize = 14; Text = Info.Text; TextXAlignment = Enum.TextXAlignment.Left; ZIndex = 6; Parent = ToggleInner; });
		...
		local ToggleRegion = Library:Create('Frame', { BackgroundTransparency = 1; Size = UDim2.new(0, 170, 1, 0); ZIndex = 8; Parent = ToggleOuter; });
		...
		function Toggle:Display() ToggleInner.BackgroundColor3 = Toggle.Value and Library.AccentColor or Library.MainColor; ToggleInner.BorderColor3 = Toggle.Value and Library.AccentColorDark or Library.OutlineColor; Library.RegistryMap[ToggleInner].Properties.BackgroundColor3 = Toggle.Value and 'AccentColor' or 'MainColor'; Library.RegistryMap[ToggleInner].Properties.BorderColor3 = Toggle.Value and 'AccentColorDark' or 'OutlineColor'; end;
		function Toggle:OnChanged(Func) Toggle.Changed = Func; Func(Toggle.Value); end;
		function Toggle:SetValue(Bool) Bool = (not not Bool); Toggle.Value = Bool; Toggle:Display(); for _, Addon in next, Toggle.Addons do if Addon.Type == 'KeyPicker' then if (Addon.SyncToggleState) then Addon.Toggled = Bool end; Addon:Update() end end; Library:SafeCallback(Toggle.Callback, Toggle.Value); Library:SafeCallback(Toggle.Changed, Toggle.Value); Library:UpdateDependencyBoxes(); end;
		function Toggle:Remove() for _, blank in Blanks do blank:Destroy(); end; Toggles[Idx] = nil; ToggleOuter:Destroy(); table.clear(Toggle); Groupbox:Resize(); end;
		ToggleRegion.InputBegan:Connect(function(Input) if Input.UserInputType == Enum.UserInputType.MouseButton1 and not Library:MouseIsOverOpenedFrame() then Toggle:SetValue(not Toggle.Value) Library:AttemptSave(); end; end);
		local contextmenu = Library:AddContextMenu(ToggleOuter, ToggleRegion); contextmenu:AddOption('Copy Flag', function() pcall(setclipboard, Toggle.Idx); task.wait(); Library:Notify('Copied flag to clipboard!', 2); contextmenu:Hide(); end);
		...
		Toggle:Display(); table.insert(Blanks, Groupbox:AddBlank(Info.BlankSize or 5 + 2)); Groupbox:Resize();
		Toggle.ColorPickerCount = 0; Toggle.ToggleRegion = ToggleRegion; Toggle.TextLabel = ToggleLabel; Toggle.Container = Container; setmetatable(Toggle, BaseAddons);
...
do
	...
	local WatermarkOuter = Library:Create('Frame', { BorderColor3 = Color3.new(0, 0, 0); Position = UDim2.new(0, 100, 0, -25); Size = UDim2.new(0, 213, 0, 20); ZIndex = 200; Visible = false; Parent = ScreenGui; });
	...
	local Gradient = Library:Create('UIGradient', { Color = ColorSequence.new({ ColorSequenceKeypoint.new(0, Library:GetDarkerColor(Library.MainColor)), ColorSequenceKeypoint.new(1, Library.MainColor), }); Rotation = -90; Parent = InnerFrame; });
	Library:AddToRegistry(Gradient, { Color = function() return ColorSequence.new({ ColorSequenceKeypoint.new(0, Library:GetDarkerColor(Library.MainColor)), ColorSequenceKeypoint.new(1, Library.MainColor), }); end });
	...
	Library.Watermark = WatermarkOuter; Library.WatermarkText = WatermarkLabel; Library:MakeDraggable(Library.Watermark);
...
Library.Fonts = {
	...
	{ "GothamBook", Enum.Font.GothamBook },
	{ "Gotham", Enum.Font.Gotham },
	{ "GothamMediumItalic", Enum.Font.GothamMediumItalic },
	{ "GothamBlackItalic", Enum.Font.GothamBlackItalic },
	{ "GothamBoldItalic", Enum.Font.GothamBoldItalic },
	{ "GothamLightItalic", Enum.Font.GothamLightItalic },
	{ "GothamBookItalic", Enum.Font.GothamBookItalic },
	{ "GothamItalic", Enum.Font.GothamItalic },
	{ "Roboto", Enum.Font.Roboto },
	{ "RobotoMono", Enum.Font.RobotoMono },
	{ "RobotoCondensed", Enum.Font.RobotoCondensed },
	{ "RobotoCondensedLight", Enum.Font.RobotoCondensedLight },
	{ "RobotoCondensedBold", Enum.Font.RobotoCondensedBold },
	{ "RobotoCondensedItalic", Enum.Font.RobotoCondensedItalic },
	{ "RobotoCondensedLightItalic", Enum.Font.RobotoCondensedLightItalic },
	{ "RobotoCondensedBoldItalic", Enum.Font.RobotoCondensedBoldItalic },
	{ "Ubuntu", Enum.Font.Ubuntu },
	{ "Fantasy", Enum.Font.Fantasy },
	{ "Antique", Enum.Font.Antique },
	{ "Cartoon", Enum.Font.Cartoon },
	{ "Classic", Enum.Font.Classic },
...
function Library:CreateWindow(...)
	...
	function Window:AddTab(Name)
		...
		local RightSide = Library:Create('ScrollingFrame', { BackgroundTransparency = 1; BorderSizePixel = 0; Position = UDim2.new(0.5, 4 + 1, 0, 8 - 1); Size = UDim2.new(0.5, -12 + 2, 0, Library.UISize.Height.Offset - 91); CanvasSize = UDim2.new(0, 0, 0, 0); BottomImage = ''; TopImage = ''; ScrollBarThickness = 0; ZIndex = 2; Parent = TabFrame; });
